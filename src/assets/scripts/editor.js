...
let app = new Vue({
  ...
  mounted() {
    this.currentFont =
      localStorage.getItem("fonts") || this.builtinFonts[0].value;
    this.currentColor =
      localStorage.getItem("color") || this.colorOption[1].value;
    ...
    this.editor.on("paste", (cm, e) => {
      ...
      for (let i = 0, len = e.clipboardData.items.length; i < len; ++i) {
        ...
        if (item.kind === "file") {
          ...
          axios
            .post("https://imgkr.com/api/files/upload", data, {
              headers: {
  ...
  methods: {
    renderWeChat(source) {
      let output = marked(source, {
        renderer: this.wxRenderer.getRenderer(this.status)
      });
      // 去除第一行的 margin-top
